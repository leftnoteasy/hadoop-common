...
public class TestIncreaseContainerResource {
  ...
  public void testIncreaseContainerResource() throws Exception {
    ...
    // check resource in node
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(3 * GB, 2));
  ...
  public void testReserveIncreaseContainerResource() throws Exception {
    ...
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(4 * GB, 3));
    ...
    // reserved container will not deduct node resource
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(4 * GB, 3));
    ...
    ContainerResourceIncrease increasedContext =
        alloc1Response.getIncreasedContainers().get(0);
    checkIncreaseContext(increasedContext, targetCapability);
    checkQueueResource(cs, Resource.newInstance(4 * GB, 2));
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(4 * GB, 2));
    ...
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(1 * GB, 1));

    rm.stop();
  }

  @Test(timeout = 3000000)
  public void testReleaseContainerWithReservedIncreaseRequest()
      throws Exception {
    /*
     ...
     * above. Then we will release c_1, we will check if reserved spaces are
     * freed
     */
    YarnConfiguration conf = new YarnConfiguration();
    MockRM rm = new MockRM(conf);
    ...
    // we will do a check on used resource here for queue
    checkQueueResource(cs, Resource.newInstance(6 * GB, 3));
    // reserved resource not reflect in node
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(4 * GB, 3));
  ...
  public void testUpdateResourceIncreaseRequestForReserved() throws Exception {
     ...
     * satisfied 2) Submit a increase request that can be satisfied, see if it's
    ...
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(2 * GB, 2));
    ...
    checkNodeUsedResource(cs, nm1.getNodeId(), Resource.newInstance(5 * GB, 2));
  ...
  public void testUpdateResourceToCancelReservedResource() throws Exception {
    ...
    FiCaSchedulerApp application =
        cs.getApplication(containerId1.getApplicationAttemptId());
    FiCaSchedulerNode node = cs.getNode(nm1.getNodeId());
    Assert.assertTrue(node.isReserved());
    Assert.assertTrue(node.getReservedIncreaseRequest() != null
        && node.getReservedIncreaseRequest().getContainerId()
            .equals(containerId1));
    Assert.assertNotNull(application.getResourceIncreaseRequest(
        node.getNodeID(), containerId1));
    ...
    Assert.assertFalse(node.getReservedIncreaseRequest() != null
        && node.getReservedIncreaseRequest().getContainerId()
            .equals(containerId1));
    Assert.assertNull(application.getResourceIncreaseRequest(node.getNodeID(),
        containerId1));
  ...
  private void checkNodeUsedResource(CapacityScheduler scheduler,
      NodeId nodeId, Resource resource) {
    FiCaSchedulerNode node = scheduler.getNode(nodeId);
    Assert.assertEquals(resource, node.getUsedResource());
  }
  ...
  private ContainerId checkAllocateResource(MockAM am, MockNM nm,
    ...
    if (expectAllocated) {
      ...
      ContainerId containerId =
          alloc1Response.getAllocatedContainers().get(0).getId();
  ...
  private void
      checkIncreaseResource(MockAM am, MockNM nm, ContainerId containerId,
          Resource targetResource, boolean expectIncreased) throws Exception {
    ...
    if (expectIncreased) {
      Assert.assertEquals(1, alloc1Response.getIncreasedContainers().size());
      ContainerResourceIncrease increasedContainer =
          alloc1Response.getIncreasedContainers().get(0);
  ...
  private void checkIncreaseContext(ContainerResourceIncrease increaseContext,
      Resource expectedResource) throws Exception {
    Assert.assertEquals(expectedResource, increaseContext.getCapability());

    // check token received
    ContainerTokenIdentifier ti =
        BuilderUtils.newContainerTokenIdentifier(increaseContext
            .getContainerToken());
